 ...
 */
public class MarkTriangle extends Mark implements Defines {
    //{{{ MarkTriangle constructor
    /**
     *  Constructor for the MarkTriangle object
     *
     *@param  x  X coordinate of the mark
     *@param  y  Y coordinate of the mark
     */
    public MarkTriangle(int x, int y) {
        this.x = x;
        this.y = y;
    } //}}}
    ...
    public void drawShape(Graphics2D g, int x, int y, int w, int h) {
        int sizeX = (int)(w * 0.7) / 2;
        int sizeY = (int)(h * 0.7) / 2;
        y -= 2;

        g.draw(new Line2D.Double(x, y - sizeY, x + sizeX, y + sizeY));
        g.draw(new Line2D.Double(x, y - sizeY, x - sizeX, y + sizeY));
